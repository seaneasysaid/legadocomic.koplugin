...
- **磁盘缓存**：图片按 URL 去重落盘，LRU 限额自动清理（默认 300MB，可调 50–2000MB），重看不耗流量
...
```
书架                     （首次使用点「⟳ 刷新书架」联网获取）
 ├── ⭐ 收藏区           长按书加/取消收藏，收藏的书置顶直达
 ├── ⭐ 收藏管理          纯点击切换收藏（推荐，比长按稳）
 └── 书名 → 章节列表 → 点章节进入阅读器
```
...
**Q：缓存会无限变大吗？**
不会。超过「缓存上限」自动按最久未访问清理；也可在设置里一键「清空图片缓存」（显示当前用量）。
