...
- **双向进度同步**：本地记住每本书读到「第几话第几张」（书架标注）；退出阅读器时自动把进度写回阅读 APP，打开书时也会接着 APP 上较新的那份进度——续读项标注 `[APP]` / `[本机]` 区分来源
- **收藏快捷方式**：⭐ 收藏常用漫画，独立「收藏」视图直达章节列表；「✎ 收藏管理」纯点击切换，不依赖长按手势
- **两级菜单**：顶层固定只有「⭐ 收藏 / ☰ 书架 / ⚙ 设置」三个入口，书全部收进子菜单——书架上有 300 本也不会把入口挤到第二页
- **只看漫画**：按阅读 APP 的书架类型标记自动识别漫画，小说/本地书不再混进书架；开关在书架列表顶部（`◉ 只看漫画: 开`），默认开，点一下即可切回显示全部，底部会提示隐藏了多少本
...
书架（顶层，永远只有 3 个入口，书再多也不变）
 ...
 │                       ⭐ 我收藏的书 → 章节列表 → 点章节进入阅读器
 ├── ☰ 书架 (N 本)   ──→  ◉ 只看漫画: 开／关（点一下切换，默认开）
 │                       ⟳ 刷新书架（首次使用点这里联网获取）
 │                       漫画 → 章节列表 → 点章节进入阅读器
 │                       » 已隐藏 N 本非漫画 · 点此显示全部（开关为开时出现）
 └── ⚙ 设置          服务器 / 预取张数 / 缓存上限 / 自动删旧章节 …
```

「收藏」和「书架」是两个独立视图：书架默认只列**漫画**（已收藏的前面带 ⭐），收藏里只列你标了星的书（不过滤，你的收藏永远可达）。已收藏的书不会因为过滤而消失，两个入口随时切换。
...
`X` 逐层返回：阅读器 → 章节 → 收藏管理 → 收藏/书架 → 顶层（收藏 / 书架 / 设置），在顶层按 `X` 退出插件。每个子菜单**第一项**就是「← 返回」，点它直接回上一层——放在最前面，所以书再多也不会被挤到第二页。
...
**Q：手机上读到一半，换电纸书能接着看吗？**
可以。插件打开书时会比较「本机进度」和「APP 进度」，取较新的那份作为续读点（续读项后用 `[APP]` / `[本机]` 标出来源）。注意：APP 进度读自**书架缓存**，想接着手机上的最新进度，请先在「☰ 书架」点一次「⟳ 刷新书架」再开书。

**Q：某本漫画在书架里找不到？**
默认开着「只看漫画」，识别依据是阅读 APP 书架里的类型标记。个别书源没被 APP 标记成图片类的书会被隐藏——点书架顶部的「◉ 只看漫画」切成关，就能看到全部书。
