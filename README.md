...
- **收藏快捷方式**：⭐ 收藏常用漫画，独立「收藏」视图直达章节列表；「✎ 收藏管理」纯点击切换，不依赖长按手势
- **两级菜单**：顶层固定只有「⭐ 收藏 / ☰ 书架 / ⚙ 设置」三个入口，书全部收进子菜单——书架上有 300 本也不会把入口挤到第二页
...
```
书架（顶层，永远只有 3 个入口，书再多也不变）
 ├── ⭐ 收藏 (N 本)  ──→  ✎ 收藏管理（点击书名加/取消收藏）
 │                       ⭐ 我收藏的书 → 章节列表 → 点章节进入阅读器
 ├── ☰ 书架 (N 本)   ──→  ⟳ 刷新书架（首次使用点这里联网获取）
 │                       全部书籍 → 章节列表 → 点章节进入阅读器
 └── ⚙ 设置          服务器 / 预取张数 / 缓存上限 / 自动删旧章节 …
```

「收藏」和「书架」是两个独立视图：书架里列**全部**书（已收藏的前面带 ⭐），收藏里只列你标了星的书。已收藏的书不会从书架里消失，两个入口随时切换。
...
`X` 逐层返回：阅读器 → 章节 → 收藏管理 → 收藏/书架 → 顶层（收藏 / 书架 / 设置），在顶层按 `X` 退出插件。每个子菜单末尾也有「← 返回」，点它直接回上一层。

## 常见问题

**Q：书架是空的？**
先进「☰ 书架 (0 本)」再点「⟳ 刷新书架」。仍为空请检查：① 阅读 APP 的 Web 服务是否已启动；② 手机和阅读器是否同一局域网；③ 服务器地址是否带端口（`http://IP:1122`）。
